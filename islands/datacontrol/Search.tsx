    ...
    <div class="relative z-20" ref={searchContainerRef}>
      {open && (
        <div class="absolute right-0 flex items-center z-20">
          <input
            type="text"
            class="min-w-[260px] mobileMd:min-w-[340px] mobileLg:min-w-[380px] desktop:min-w-[460px] h-7 mobileMd:h-9 desktop:h-[42px] bg-stamp-purple-bright pl-3 pr-2 py-3 rounded-md text-sm mobileLg:text-base leading-tight text-black font-medium placeholder:text-stamp-purple-darkest placeholder:font-medium placeholder:uppercase outline-none"
            placeholder={placeholder}
            value={searchTerm}
            onInput={(e) => setSearchTerm((e.target as HTMLInputElement).value)}
          />
          {results.length > 0 && (
            <ul class="absolute top-full right-0 w-full bg-stamp-purple-bright rounded-b-md text-black font-bold text-xs mobileLg:text-sm leading-tight uppercase z-20 max-h-[162px] mobileLg:max-h-[192px] overflow-y-auto scrollbar-purple">
              {results.map((result) => (
                <li
                  key={result.tick || JSON.stringify(result)}
                  onClick={() => onResultClick(result)}
                  class="cursor-pointer pl-3 py-1.5 hover:bg-stamp-purple"
                >
      ...
      <Button
        ...
        class={`cursor-pointer ${open ? "invisible" : ""}`}
        onClick={handleOpen}
        role="button"
        aria-label="Search"
      />
