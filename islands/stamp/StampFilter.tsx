...
// Change from export const to function declaration
function filtersToQueryParams(search: string, filters: any) {
  const url = new URL(
  ...
  // Set file type params
  Object.entries(filters.fileType).forEach(([key, value]) => {
    if (value) {
      params.set(`fileType[${key}]`, "true");
    }
  });

  // Set editions params
  Object.entries(filters.editions).forEach(([key, value]) => {
    if (value) {
      params.set(`editions[${key}]`, "true");
    }
  });
  ...
  // Set market params
  Object.entries(filters.market).forEach(([key, value]) => {
    if (key === "priceRange") {
      if (value.min) params.set("market[priceRange][min]", value.min);
      if (value.max) params.set("market[priceRange][max]", value.max);
    } else if (value) {
      params.set(`market[${key}]`, "true");
    }
  });
...
export const StampFilter = ({
  ...
  const [filters, setFilters] = useState(() =>
    queryParamsToFilters(searchparams.toString())
  );
  ...
  const handleFilterChange = (key: string, value: any) => {
    ...
    // Update URL using the exported filtersToQueryParams function
  ...
  const handleClearFilters = () => {
    setFilters(queryParamsToFilters(searchparams.toString()));
    const queryString = filtersToQueryParams(
      globalThis.location.search,
      queryParamsToFilters(searchparams.toString()),
    );
