...
export async function handleCache(key: string, fetchFunction: () => Promise<any>, ttl: number | 'never') {
...
export async function handleSqlQueryWithCache(client: Client, query: string, params: any[], ttl: number | 'never') {
    const cacheKey = generateSQLCacheKey(query, params);
    return handleCache(cacheKey, () => handleQueryWithClient(client, query, params), ttl);
}
