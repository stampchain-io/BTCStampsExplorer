...
Deno.test('Utils Types - Function and Callback Types', () => {
  ...
  const callbackWithParam: CallbackWithParam<string, number> = (param: string) => param.length;
  const asyncCallbackWithParam: AsyncCallbackWithParam<number, string> = async (param: number) => param.toString();
