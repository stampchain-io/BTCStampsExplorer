...
static async get_total_cursed_by_ident_with_client(client: Client, ident: SUBPROTOCOLS){
  return await handleSqlQueryWithCache(
    client,
    `
    SELECT COUNT(*) AS total
...
static async get_cursed_by_page_with_client(client: Client, limit = 1000, page = 0){
...
static async get_resumed_cursed_by_page_with_client(client: Client, limit = 1000, page = 1, order = "DESC"){
...
static async get_cursed_by_block_index_with_client(client: Client, block_index: number){
  return await handleSqlQueryWithCache(
    client,
    `
    SELECT st.*, cr.creator AS creator_name
...
static async get_cursed_by_ident_with_client(client: Client, ident: SUBPROTOCOLS, limit = 1000, page = 0){
  const offset = limit && page ? Number(limit) * (Number(page) - 1) : 0;
  return await handleSqlQueryWithCache(
    client,
    `
