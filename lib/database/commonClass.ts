...
export class CommonClass {
  ...
  static async get_related_blocks_by_hash_with_client(
    ...
    const block_index = await this.get_block_index_by_hash_with_client(client, block_hash);
    const blocks = await handleSqlQueryWithCache(
      client,
      `
      SELECT * FROM blocks
  ...
  static async get_total_stamp_balance_with_client(client: Client, address: string){
    try{
  ...
  static async get_stamp_balances_by_address_with_client(
    ...
    try {
      ...
      return summarized.map(summary => {
        const xcp_balance = xcp_balances.find((balance: any) => balance.cpid === summary.cpid);
        return {
          ...summary,
