...
class DependencyGraphAnalyzer {
  ...
  private importPatterns = {
    es6Import: /import\s+(?:type\s+)?(?:\{[^}]*\}|\*\s+as\s+\w+|\w+)(?:\s*,\s*(?:\{[^}]*\}|\*\s+as\s+\w+|\w+))*\s+from\s+["']([^"']+)["']/g,
    dynamicImport: /import\s*\(\s*["']([^"']+)["']\s*\)/g,
    requireImport: /require\s*\(\s*["']([^"']+)["']\s*\)/g,
    typeImport: /import\s+type\s+(?:\{[^}]*\}|\*\s+as\s+\w+|\w+)\s+from\s+["']([^"']+)["']/g
  };
  ...
  async generateVisualization(format: "mermaid" | "dot" | "json" = "mermaid"): Promise<string> {
    switch (format) {
      case "mermaid":
  ...
  private async parseImportExportRelationships(): Promise<void> {
    ...
    for (const modulePath of this.typeModules) {
      ...
      try {
        ...
        console.warn(`   Warning: Failed to parse ${modulePath}: ${error.message}`);
      }
    }
  ...
  private async parseModule(modulePath: string, content: string): Promise<DependencyNode> {
    ...
    const exportMatches = content.match(/export\s+(?:type\s+|interface\s+|class\s+|function\s+|const\s+|let\s+|var\s+)?(\w+)/g) || [];
    exports.push(...exportMatches.map(match => match.replace(/export\s+(?:type\s+|interface\s+|class\s+|function\s+|const\s+|let\s+|var\s+)?/, '')));
  ...
  private resolveImportPath(fromModule: string, importPath: string): string | null {
  ...
  private determineModuleType(modulePath: string): "client" | "server" | "shared" | "test" {
    ...
    if (modulePath.startsWith('islands/') || modulePath.startsWith('components/') || 
  ...
  private buildDependencyGraph(): void {
    ...
    console.log(`   Built graph with ${this.graph.nodes.size} nodes and ${Array.from(this.graph.edges.values()).flat().length} edges`);
  }
  ...
  private detectCircularDependencies(): void {
    ...
    for (const circularDep of this.graph.circularDependencies) {
      for (const nodeId of circularDep.cycle) {
        ...
        if (node) {
          node.cyclicDependencies.push(...circularDep.cycle.filter(id => id !== nodeId));
        }
      }
    }

    console.log(`   Found ${this.graph.circularDependencies.length} circular dependencies`);
  }

  private calculateCycleSeverity(cycle: string[]): "critical" | "major" | "minor" {
    // Critical: cycles involving more than 5 modules or server/client boundary crossing
    if (cycle.length > 5) return "critical";
    
    const moduleTypes = cycle.map(nodeId => this.graph.nodes.get(nodeId)?.moduleType || "shared");
    const hasServerClient = moduleTypes.includes("server") && moduleTypes.includes("client");
      ...
      if (node.moduleType === "client") {
        ...
        for (const depId of node.dependencies) {
          ...
          if (depNode?.moduleType === "server") {
            leaks.push(`Client module ${nodeId} imports server module ${depId}`);
          }
        }
        ...
        for (const depId of node.dependencies) {
          ...
          if (depNode?.moduleType === "client") {
            leaks.push(`Server module ${nodeId} imports client module ${depId}`);
          }
        }
      }
    }
  ...
  private calculateHealthScores(): void {
    ...
    for (const [nodeId, node] of this.graph.nodes) {
      ...
      if (node.moduleType === "client" && node.dependencies.some(dep => 
  ...
  private generateHealthReport(): void {
    ...
    const healthScores = Array.from(this.graph.nodes.values()).map(node => node.healthScore);
    ...
    if (circularDependencies > 0) {
      criticalIssues.push(`${circularDependencies} circular dependency cycles detected`);
      recommendations.push("Break circular dependencies by extracting interfaces or shared modules");
    }

    if (clientServerLeaks > 0) {
      criticalIssues.push(`${clientServerLeaks} client-server boundary violations`);
      recommendations.push("Ensure client code doesn't import server modules and vice versa");
    }
    ...
    if (averageHealthScore < 70) {
      criticalIssues.push(`Low average health score: ${averageHealthScore.toFixed(1)}`);
      recommendations.push("Refactor modules with low health scores to improve maintainability");
    }
  ...
  private generateMermaidDiagram(): string {
    ...
    for (const circularDep of this.graph.circularDependencies) {
      if (circularDep.severity === "critical") {
        for (let i = 0; i < circularDep.cycle.length; i++) {
          ...
          const toId = circularDep.cycle[(i + 1) % circularDep.cycle.length].replace(/[\/\-\.]/g, '_');
    ...
    lines.push("    classDef critical fill:#ffebee,stroke:#f44336,stroke-width:3px");
  ...
  private generateDotGraph(): string {
    ...
    for (const [nodeId, node] of this.graph.nodes) {
      const shortId = nodeId.replace(/[\/\-\.]/g, '_');
      const displayName = nodeId.split('/').pop() || nodeId;
      const color = node.moduleType === "client" ? "lightblue" : 
                   node.moduleType === "server" ? "lightgreen" : 
                   node.moduleType === "test" ? "lightyellow" : "lightgray";
      
      lines.push(`  ${shortId} [label="${displayName}", fillcolor="${color}", style=filled];`);
    }
  ...
  printReport(): void {
    ...
    if (worstNodes.length > 0 && worstNodes[0].healthScore < 80) {
      ...
      worstNodes.forEach(node => {
        ...
        if (node.cyclicDependencies.length > 0) {
          console.log(`      🔄 ${node.cyclicDependencies.length} circular deps`);
        }
        if (node.dependencies.length > 10) {
    ...
    // Overall status
    const overallHealth = report.criticalIssues.length === 0 && report.averageHealthScore >= 80;
    console.log(`\n${overallHealth ? "✅" : "❌"} Overall Status: ${overallHealth ? "HEALTHY" : "NEEDS ATTENTION"}`);
  }
}
...
if (import.meta.main) {
  ...
  const format = args.find(arg => arg.startsWith("--format="))?.split("=")[1] as "mermaid" | "dot" | "json" || "mermaid";
  ...
  try {
    ...
    // Save visualization
    const visualizationPath = join(reportsDir, `dependency-graph.${format === "json" ? "json" : format === "dot" ? "dot" : "mmd"}`);
...
export { DependencyGraphAnalyzer, type DependencyGraph, type CircularDependency };