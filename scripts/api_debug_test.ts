...
const EXPECTED_DB_RESPONSE = {
    ...
    deploy_img: "https://stampchain.io/stamps/e90e6608c68a3264b202fb2f2e7abf780be454b98aa107b75e9c7ab9dc94244e.svg"
};
...
async function testBalanceEndpoint() {
    ...
    for (const testCase of testCases) {
        ...
        if (testCase.tick) {
            console.log("Tick parameter transformation:");
            console.log("Original tick:", testCase.tick);
            console.log("Unicode escaped:", emojiToUnicodeEscape(testCase.tick));
        }
        ...
        try {
            ...
            const balanceParams = {
                address: testCase.address,
                tick: testCase.tick ? emojiToUnicodeEscape(testCase.tick) : undefined,
            ...
            if (DEBUG && testCase.tick) {
                ...
                console.log("Database Query Result:", JSON.stringify(dbResult, null, 2));
            }
            
            // Then test controller layer
            console.log("\nTesting controller layer:");
            const result = await Src20Controller.handleSrc20BalanceRequest(balanceParams);
            console.log("Controller Response:", JSON.stringify(result, null, 2));
            
            // Compare with expected format
            if (testCase.tick === "luffy") {
                console.log("\nComparing with expected formats:");
                console.log("Expected empty response format:", JSON.stringify(EXPECTED_EMPTY_RESPONSE, null, 2));
                console.log("Expected DB response format:", JSON.stringify(EXPECTED_DB_RESPONSE, null, 2));
                
                // Log emoji conversion details
                console.log("\nEmoji conversion test:");
                const unicodeEscaped = emojiToUnicodeEscape(testCase.tick);
                console.log("Original -> Unicode:", `${testCase.tick} -> ${unicodeEscaped}`);
                console.log("Unicode -> Emoji:", `${unicodeEscaped} -> ${unicodeEscapeToEmoji(unicodeEscaped)}`);
            }
