...
Deno.test("BTC Price Caching System - Performance Validation", async (t) => {
  ...
  await t.step("Price consistency across multiple calls", async () => {
    ...
    prices.forEach((price, index) => {
      assertEquals(
        price,
        firstPrice,
        `Call ${index + 1} returned different price: ${price} vs ${firstPrice}`,
      );
    });
  ...
  await t.step("API call reduction validation", async () => {
    ...
    console.log(`✅ API call reduction validated - using centralized caching`);
  });
});
...
Deno.test("BTC Price Caching System - Integration Tests", async (t) => {
  ...
  await t.step("Fallback behavior validation", async () => {
    ...
    const fallbackPrice = await fetchBTCPriceInUSD("http://localhost:99999");
    assertEquals(fallbackPrice, 0, "Should return 0 when endpoint unavailable");
...
Deno.test("BTC Price Caching System - Performance Benchmarks", async (t) => {
  await t.step("Response time benchmarks", async () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      times.push(time);
      assert(price > 0, `Iteration ${i + 1} returned invalid price: ${price}`);
    }
    ...
    assert(avgTime < 1000, `Average response time too slow: ${avgTime}ms`);
    assert(maxTime < 2000, `Max response time too slow: ${maxTime}ms`);
