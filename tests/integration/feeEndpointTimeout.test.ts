...
Deno.test("Fee Endpoint Integration Tests", async (t) => {
  ...
  await t.step("Fee endpoint should respond quickly in development mode", async () => {
    ...
    try {
      ...
      assertEquals(duration < 2000, true, `Endpoint should respond in < 2s, got ${duration}ms`);
      ...
      assertEquals(data.source, "default", "Should use default source in development");
      assertEquals(data.fallbackUsed, true, "Should use fallback in development");
      assertEquals(data.recommendedFee >= 1, true, "Recommended fee should be >= 1 sat/vB");
      ...
      if (error instanceof Error && error.name === "AbortError") {
        throw new Error(`Fee endpoint timed out after 5 seconds - this indicates the timeout issue is not fixed`);
      }
      throw error;
    }
  });

  await t.step("Fee endpoint should handle CSRF validation properly", async () => {
    ...
    assertEquals(response.ok, true, "Should succeed without CSRF token in development");

    const data = await response.json();
    assertExists(data.recommendedFee, "Should return fee data");
  });

  await t.step("Fee endpoint should return consistent data structure", async () => {
    // Make multiple requests to ensure consistency
    const requests = Array.from({ length: 3 }, () =>
      fetch("http://localhost:8000/api/internal/fees").then(r => r.json())
    );
    ...
    console.log(`✅ Made ${results.length} requests, all returned consistent data structure`);
  });

  await t.step("Fee endpoint should handle rapid requests without issues", async () => {
    ...
    assertEquals(avgResponseTime < 500, true, `Average response time should be < 500ms, got ${avgResponseTime}ms`);
    ...
    console.log(`✅ Completed ${results.length} rapid requests in ${totalDuration}ms (avg: ${avgResponseTime.toFixed(1)}ms per request)`);
  });
...
Deno.test("Fee Service Development Mode Tests", async (t) => {
  await t.step("Development mode environment variables should be set correctly", () => {
    ...
    if (currentEnv === "development") {
      assertEquals(skipRedis, true, "SKIP_REDIS_CONNECTION should be true in development");
    }
  });
